...
'HELP_MAIN':'Crea una funcion de inicio "main" en el ultimo fichero para poder ejecutar el codigo directamente.',	
'HELP_OUT':'Carpeta donde se guardará los ficheros generados, por defecto es el directorio actual.',	
'HELP_COMMENTS':'Los comentarios dentro del codigo Perl, se mantendran en el codigo java.',	
'HELP_EMULATE_PAREN':'Añade automaticamente los parentesis a las funciones, si el codigo es sintacticamente correcto, deberia hacerlo correctamente.',	
'HELP_OPTIMIZE_CODE':'Mejora el codigo de salida haciendolo mas visible y eliminado redundancias dando lugar a un mayor rendimiento.',	
'HELP_UNRECHEABLE_CODE':'Comprueba la existencia de codigo muerto, si existe, el codigo resultante no podra ser copilado.',	
'HELP_ERROR_ABORT':'Para el analisis en caso de encontrar un error.',	
'HELP_DEBUGGER':'Opciones para la depuracion del analisis.',	
'HELP_DEBUGGER_LEXER':'Activa la impresion de tokens para el analizador lexico.',	
'HELP_DEBUGGER_PARSER':'Activa la impresion de reglas para el analizador sintactico',	
'HELP_DEBUGGER_SIZE':'Tamaño del codigo mostrado para cada regla durante la depuracion.',	
'HELP_DEBUGGER_FILE':'Fichero para almacenar la depuracion de reglas para mejorar su visivilidad y compresion.',	
'HELP_DEBUGGER_DETAILS':'Si se usa un fichero, podran mostrarse mas detalles sobre las reglas al analizar.',			
#Errores Interfaz	
'FILE_NOT_FOUND':'No se ha encontrado el fichero %file.',
'FILE_NOT_ACCESS':'No se ha podico acceder al fiero %file.',
'OUT_NOT_FOUND':'El directorio de salida no existe.',
'OUT_NOT_ACCESS':'No tienes permiso de escritura en el directorio de salida.',		
#Errores Generales
'ERROR':'Error',
'WARNING':'Aviso',
'FILE_NOT_FOUND':'Archivo %file no encontrado o no ha sido posible acceder a él.',
'ILEGAL_TOKEN':'El caracter %c no está permitido',
...
def get_message(id):
	...
	if LANGUAGE in languages and id in languages[LANGUAGE]:
		return languages[LANGUAGE][id]
	else:
		return languages[DEFAULT_LANG][id]	


def error(parser=None,error=None,position=None,**names):
	...
	if parser:	
		#Si solo es un aviso
		if error in not_fatal:
			#Imprimimos el mensaje como aviso
			print(parser.file_name+':'+msg_pos+get_message('WARNING')+': '+msg)
			show_code(parser,position)
		else:
			#Imprimimos el mensaje como error
			print(parser.file_name+':'+msg_pos+get_message('ERROR')+': '+msg)
			show_code(parser,position)
		...
		if error in not_fatal:
			#Imprimimos el mensaje como aviso
			print(get_message('WARNING')+': '+msg)
		else:
			#Imprimimos el mensaje como error
			print(get_message('ERROR')+': '+msg)		
	
def show_code(parser,pos):	
	#Si sabemos la posicion
	if pos and pos.lexpos:
		#Tamaño maximo a ambos lados del error
		TAM=40
		...
		#Si hay fin de linea, cortamos por ahí
		init=lcode.rfind("\n")+1
		end=rcode.find("\n")
		if end==-1: end=TAM*2
